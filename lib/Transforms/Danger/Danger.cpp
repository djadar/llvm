	...
	virtual bool runOnFunction(Function &F) {
		...
		for(Function::iterator bb = F.begin(), e = F.end(); bb != e; ++bb) {
			...
			for(BasicBlock::iterator i = bb->begin(), e = bb->end(); i != e; ++i) {
				...
				if(PointerType *pointerType=dyn_cast<PointerType>(allocaInst->getOperand(0)->getType()->getPointerElementType() )){
						errs() << "1 un pointeur"<<'\n';
						//errs()<< allocaInst->getOperand(0)->getType()->getArrayElementType() <<"\n";
